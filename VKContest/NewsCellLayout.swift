...
struct NewsCellLayout {
    ...
    mutating func calculateLayoutFitting(_ size: CGSize) {
        ...
        self.size = CGSize(
            width: size.width,
            height: self.headerViewLayout.size.height + self.barViewLayout.size.height
        )
        self.headerViewFrame = CGRect(origin: .zero, size: self.headerViewLayout.size)
        self.barViewFrame = CGRect(origin: CGPoint(x: 0, y: self.headerViewFrame.maxY), size: self.barViewLayout.size)
    }
}
